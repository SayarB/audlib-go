...
func AuthMiddleware() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		fmt.Println("Claims: ", claims.Subject)

		user := &models.User{}
		err := database.GetUserByClerkId(claims.Subject, user)
		...
		org := &models.Organization{}
		fmt.Println("org_id = " + claims.ActiveOrganizationID)
		err = database.GetOrganizationByClerkId(claims.ActiveOrganizationID, org)

		if err != nil {
			return c.Status(401).JSON(fiber.Map{"message": "Organization not found"})
		}
